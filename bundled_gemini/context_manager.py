...
import os         # 👇 新增
import mimetypes  # 👇 新增
from .config import ACCOUNTS, state
        ...
        try:
            if cmd == "/account":
                if len(parts) > 1 and parts[1] in ACCOUNTS:
                    ...
                    success, msg = await gemini_conn.initialize()
                    if success:
                ...
                return True, f"🛠️ **指令**:\n`/account <1|2>` : 切换账号\n`/model <代号>` : 切换模型\n\n**状态**: 账号 `{state.active_account}` | 模型 `{state.active_model}`"
                    ...
                    for part in content:
                        if part.get("type") == "text":
                            ...
                            if "base64," in image_url:
                                # 👇 修复：动态推断真实文件扩展名并生成本地临时文件
                                ext = ".png"
                                if image_url.startswith("data:"):
                                    mime_part = image_url.split(";base64,")[0]
                                    mime_type = mime_part[5:]  # 获取例如 'image/jpeg'
                                ...
                                with os.fdopen(fd, 'wb') as f:
                                    f.write(image_bytes)
                                extracted_files.append(path) # 传递文件路径，而非 BytesIO
                else:
                    prompt_text += content + "\n"
                prompt_text += "\n"
            elif role == "assistant":
